...
var UtilFn = {
    ...
    ajaxInvokeService: function(options) {
        ...
        if (options.isRedirect) {
            options.redirect = options.redirect || function() {
        ...
        options.error = options.error || function(jqXHR) {
            if (jqXHR.responseJSON && jqXHR.responseJSON.message) {
                UtilFn.showMessageToast(jqXHR.responseJSON.message, 4000, options.redirect);
            } else {
                UtilFn.showMessageToast((options.messageTimeOut || 'Tiempo de espera agotado'), 4000, function() {
                    localStorage.removeItem('usuario');
    ...
    buildModalDataImportant: function() {
        ...
        var html = '<div id="modalInformacionImportante" class="modal modal-fixed-footer">' +
    ...
    windowSelectionMedia: function(idElement, fnCamara, fnGaleria) {
        ...
        var html = '<div id="modalSeleccionCamaraPick" class="modal">' +
            '<div class="modal-content">' +
                        ...
                        switch (type) {
                            case 'email':
                                ...
                                if (!filterEmail.test($("#" + idElement).val())) {
                                    UtilFn.showMessageToast(messageErrorEmail || "Email invalido.");
                                    $("#" + idElement).focus();
                                    validate = false;
                                    errors++;
                                }
                                break;
                            case 'number':
                                $("#" + idElement).val(($("#" + idElement).val() ? $("#" + idElement).val().trim() : ''));
                                if (isNaN($("#" + idElement).val())) {
                                    UtilFn.showMessageToast(messageErrorNumber || "Formato numérico invalido.");
                                    $("#" + idElement).focus();
                                    validate = false;
                                    errors++;
                                }
    ...
    selectCatalogos: function(grupo, idSelect, displayName, value, emptyText, disabled) {
        UtilFn.buildComboSelect({
            ...
            success: function(httpResponse) {
                ...
                if (data) {
                    ...
                    var firstItem = '<option ' + (disabled ? 'disabled="disabled"' : '') + ' value="" ' + (value ? '' : 'selected') + '>' + (emptyText ? emptyText : 'Seleccione') + '</option>';
    ...
    buildMapaGoogle: function(map, idDom, searchBox, idInput, markers, fn_callback, marker, iconUrl, zoom) {
        navigator.geolocation.getCurrentPosition(
            function(position) {
                UtilFn.createMapaGoogle(position, map, idDom, searchBox, idInput, markers, fn_callback, marker, iconUrl, zoom);
            },
            function() {}, {
                enableHighAccuracy: true
            }
    ...
    modalInfoErrorGpsAndNetwork: function(title, msg, errorType) {
        swal({
            ...
            function(isConfirm) {
                try {
                    var redirectTemp = window.location.href.substr(window.location.href.lastIndexOf('/') + 1);
    ...
    checkEnabled: function() {
        ...
        if (Connection.NONE == networkState) {
            ...
            UtilFn.modalInfoErrorGpsAndNetwork("Error de Red", "Existe un error de conectividad.");
            return;
        ...
        cordova.plugins.diagnostic.isLocationAvailable(
            function(available) {
                ...
                if (!available) {
                    UtilFn.modalInfoErrorGpsAndNetwork("Error GPS", "por favor active el GPS de su dispositivo.");
                }
            },
            function(error) {
                if (!available) {
    ...
    createMapaGoogle: function(position, map, idDom, searchBox, idInput, markers, fn_callback, marker, iconUrl, zoom) {
        ...
        UtilFn.buildComboSelect({
            ...
            success: function(httpResponse) {
                ...
                if (data) {
                    ...
                    var firstItem = '<option  value="" ' + (disabled ? 'disabled' : '') + ' selected >' + (options.textEmpty ? options.textEmpty : 'Seleccione el distrito') + '</option>';
                    $('#' + idDom).append(firstItem);
                    $.each(data, function(i, item) {
                        $('#' + idDom).append('<option ' + (item.catastro ? 'data-catastro = "' + item.catastro + '"' : '') + ' value="' + item.id + '">' + item.nombre + '</option>');
                    });
                    if (value) {
                        $('#' + idDom).val(value);
                    }
    ...
    refreshCountNotificaciones: function() {
        ...
        UtilFn.ajaxInvokeService({
            ...
            success: function(response) {
                    $('.countNotifiacionesView').html(response.data > 9 ? '9+' : response.data);
                }
                //error invoke
                ,
            error: function() {}
