...
(function() {
    ...
    buscoService.formatoFechaPerdida = function(item) {
        ...
        if (minutos < 60) {
            textPerdida = (minutos <= 1 ? 'Se perdió hace un minuto.' : 'Se perdió  hace ' + minutos + ' minutos.')
        } else {
            if (horas >= 1 && horas < 24) {
                textPerdida = (horas == 1 ? 'Se perdió hace una hora.' : 'Se perdió hace ' + horas + ' horas.')
            } else {
                textPerdida = 'Se perdió el ' + moment(new Date(item.fechaPerdida)).format('DD/MM/YYYY') + ' a las ' + moment(new Date(item.fechaPerdida)).format('h:mm a')
            }
        }
        return textPerdida;
    }
    ...
    buscoService.buildList = function(data) {
        ...
        data.forEach(function(item, index) {
            ...
            html = ' <li class="collection-item avatar "> ' +
                ' <img src="' + (item.fotoBusqueda && item.fotoBusqueda.foto ? BASE_URL_SERVER_WEB_IMAGES + item.fotoBusqueda.foto : 'images/pets_default.png') + '" alt="" style="margin-top:10px;border-radius: 0px !important;" class="circle"> ' +
    ...
    buscoService.showDetalle = function(detalle) {
        ...
        $('#decripcionDetalle').html(detalle.descripcion + (detalle.desDistrito ? '<br><b>Distrito</b>: ' + detalle.desDistrito : '') + '<br><b>' + buscoService.formatoFechaPerdida(detalle) + '</b>');
        $('#decripcionDetalle').append('<br>Recompensa: ' + detalle.montoRecompensa);
        ...
        $('#telefonoPersonaDetalle').html(
            (detalle.telefono ?
                detalle.telefono :
                (detalle.usuario && detalle.usuario.persona ? detalle.usuario.persona.contactoPrincipal.numero : '')
            )
        );
        ...
        setTimeout(function() {
            ...
            $("#btnCompartir").click(function() {
                UtilFn.compartirEnlace({
                    ...
                    mensaje: 'Se perdió, ' + (detalle.nombre ? detalle.nombre : 'engreido sin nombre.') + ' de raza ' + (detalle.desRaza ? detalle.desRaza : '') + ' cerca a ti el ' + moment(new Date(detalle.fechaPerdida)).format('DD/MM/YYYY hh:mm A'),
                    url: BASE_URL_WEB_PDP + 'mostrarPosterPublicacion?id=' + detalle.id + '&tipo=B'
                });
            });
            ...
            $("#btnDescargaPdf").click(function() {
                window.open(BASE_URL_SERVER_WEB_IMAGES + 'descargarPosterpdfBusqueda?id=' + detalle.id, '_blank', 'location=yes');
                    ...
                    (function(location) {
                        ...
                        if (distance_from_location <= radius_km * 1000) {
                            ...
                            var infowindow = new google.maps.InfoWindow({
                                content: '<h5 class="verDetalleMapa">' + location.item.nombre + '</h5>' +
                                    '<p class="flow-text verDetalleMapa">' + location.item.descripcion + '<br><br><small class="cyan-text text-darken-3" >' + buscoService.formatoFechaPerdida(location.item) + '</small></p>'
