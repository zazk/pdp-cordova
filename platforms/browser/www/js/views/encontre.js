...
(function() {
  ...
  encontreService.formatoFechaPerdida = function(item) {
    ...
    if (minutos < 60) {
      textPerdida =
        minutos <= 1
          ? 'Lo encontré hace un minuto.'
          : 'Lo encontré  hace ' + minutos + ' minutos.';
    } else {
      if (horas >= 1 && horas < 24) {
        textPerdida =
          horas == 1
            ? 'Lo encontré hace una hora.'
            : 'Lo encontré hace ' + horas + ' horas.';
      } else {
        textPerdida =
          'Lo encontré el ' +
          moment(new Date(item.fechaEncontrado)).format('DD/MM/YYYY') +
          ' a las ' +
          moment(new Date(item.fechaEncontrado)).format('h:mm a');
      }
    }
    return textPerdida;
  };
  ...
  encontreService.buildList = function(data) {
    ...
    data.forEach(function(item, index) {
      ...
      html =
        ...
        item.fotoBusqueda.foto +
        '" alt="" style="margin-top:10px;border-radius: 0px !important;" class="circle"> ' +
        ' <span class="title">' +
        (item.mascotaNombre ? item.mascotaNombre : 'Desconocido.') +
        '</span> ' +
        ' <p style="padding">' +
  ...
  encontreService.showDetalle = function(detalle) {
    ...
    $('#detalleNombrePerro').html(
      detalle.mascotaNombre ? detalle.mascotaNombre : 'Desconocido.'
    );
    ...
    $('#decripcionDetalle').html(
      detalle.descripcion +
        (detalle.desDistrito
          ...
          : '') +
        '<br><b>' +
        encontreService.formatoFechaPerdida(detalle) +
        '</b>'
    );
    ...
    $('#telefonoPersonaDetalle').html(
      detalle.telefono
        ? detalle.telefono
        : detalle.usuario &&
          detalle.usuario.persona &&
          detalle.usuario.persona.contactoPrincipal &&
    ...
    setTimeout(function() {
      ...
      $('#btnCompartir').click(function() {
        UtilFn.compartirEnlace({
          ...
          mensaje:
            'Se encontró al engreído  ' +
            (detalle.nombre ? detalle.nombre : 'sin nombre.') +
            ' de raza ' +
            (detalle.desRaza ? detalle.desRaza : '') +
            ' cerca a ti el ' +
            moment(
              new Date(
                detalle.fechaEncontrado
                  ? detalle.fechaEncontrado
      ...
      $('#btnDescargaPdf').click(function() {
        window.open(
          BASE_URL_SERVER_WEB_IMAGES +
            'descargarPosterpdfEncontrada?id=' +
            detalle.id,
          '_blank',
          'location=yes'
        );
            ...
            if (distance_from_location <= radius_km * 1000) {
              ...
              var infowindow = new google.maps.InfoWindow({
                content:
                  ...
                  (location.item && location.item.mascotaNombre
                    ? location.item.mascotaNombre
                    : 'Desconocido.') +
                  '</h5>' +
                  '<p class="flow-text verDetalleMapa">' +
                  location.item.descripcion +
                  '<br><br><small class="cyan-text text-darken-3" >' +
                  encontreService.formatoFechaPerdida(location.item) +
                  '</small></p>'
              });
